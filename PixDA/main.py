import argparse
import os
from dataloader import get_svhn, get_mnist, get_cifar10, get_stl10
...
parser.add_argument("--n_classes", type=int, default=9, help="number of classes in the dataset")
...
for epoch in range(opt.n_epochs):
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(train_source, train_target)):
        ...
        valid_a = Variable(FloatTensor(len(imgs_A), *patch).fill_(1.0), requires_grad=False)
        fake = Variable(FloatTensor(len(imgs_A), *patch).fill_(0.0), requires_grad=False)
        valid_b = Variable(FloatTensor(len(imgs_B), *patch).fill_(1.0), requires_grad=False)
        # Configure input
        ...
        # Sample noise
        z = Variable(FloatTensor(np.random.uniform(-1, 1, (len(imgs_A), opt.latent_dim))))
        ...
        g_loss = lambda_adv * adversarial_loss(discriminator(fake_B), valid_a) + lambda_task * task_loss_
        ...
        real_loss = adversarial_loss(discriminator(imgs_B), valid_b)
